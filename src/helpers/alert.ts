...
export class Alert {
  ...
  public static likeNotification(like) {
    Like.getLike(like._id).then((like) => {
  ...
  public static sharedNotification(post) {
    ...
    let idPost = post._id;
    Post.findOnePost(idPost).then((post) => {
  ...
  public static reportCloseOnly(report: any, admin) {
    Notification.newNotification({
      user: report.user._id,
      comment: report.comment._id,
      friend: admin,
      action: "report_close_only",
      routerlink: `/report/${report._id}`,
    })
      .then(() => {
        this.notification(report.user._id);
      })
      .catch((err) => {});
  }
  public static reportCloseAndDelete(report: any, admin) {
    Notification.newNotification({
      user: report.user._id,
      comment: report.comment._id,
      friend: admin,
      action: "report_close_delete",
      routerlink: `/report/${report._id}`,
    })
      .then(() => {
        this.notification(report.user._id);
